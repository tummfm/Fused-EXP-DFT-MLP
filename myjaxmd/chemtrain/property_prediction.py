...
def energy_prediction(
    ...
    def property_predictor(
            ...
            **dynamic_kwargs):
        model = model_class(r_cutoff, n_species, 1, **model_kwargs)
        per_atom_quantities = model(mol_graph, **dynamic_kwargs)
        # masked_quantities = per_atom_quantities[:, 0]*mol_graph.species_mask
        # global_properties = jnp.sum(masked_quantities, axis=0)
        n_predicted = per_atom_quantities.shape[1]
        n_per_atom = 0
        n_global = n_predicted - n_per_atom
        global_properties = jnp.sum(per_atom_quantities[:, :n_global], axis=0)
