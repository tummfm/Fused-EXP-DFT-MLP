...
def trajectory_generator_init_modified(simulator_template, energy_fn_template,
                                       ref_timings=None, quantities=None):
    """
    modified: Changed call from quantity_traj to quantity_traj_modified. This is
              because in quantity_traj_modified I changed quantities[quantity_fn_key] ->
              quantities[quantity_fn_key]['compute_fn']. Else the function is not called.
              Did not change original function as I don't want to mess around with existing
               functionalities.
    ...
    def generate_reference_trajectory(params, sim_state, **kwargs):
        """
        Returns a new TrajectoryState with auxiliary variables.
        ...
        # temperature is inexpensive and generally useful: compute it by default
        quantities['kbT'] = custom_quantity.temperature
        aux_trajectory = quantity_traj_modified(state, quantities, params)
        return state.replace(aux=aux_trajectory)

    return generate_reference_trajectory


def quantity_traj_modified(traj_state, quantities, energy_params=None):
    """
    modified: Changed quantities[quantity_fn_key] -> quantities[quantity_fn_key]['compute_fn'].
              Else the function is not called. Did not change original function as I don't want
              to mess around with existing functionalities.
    Computes quantities of interest for all states in a trajectory.
    ...
    def single_state_quantities(_, single_snapshot):
        ...
        computed_quantities = {
            quantity_fn_key: quantities[quantity_fn_key]['compute_fn'](state, **kwargs)
            for quantity_fn_key in quantities
        }
        return _, computed_quantities
