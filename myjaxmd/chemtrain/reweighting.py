...
from jax import (checkpoint, lax, random, grad, tree_map,
                 numpy as jnp)
...
def init_stiffness_loss_function(energy_fn_template, box_tensor, kbT, N, target_dict):

    # gamma_stiffness = 1.e-10
    gamma_stiffness = 1.e-6
    _, _, sigma_tensor_prod, stiffness_tensor_fn = \
    ...
    def loss_fn(quantity_trajs, weights):
        ...
        # TODO: In sitffness computation I print values this is not a pure function, might cause issues.
        stiffness_pred = custom_quantity.stiffness_tensor_components_hexagonal_crystal(stiffness_tensor)

        stress_tensor_mean = reweighting_average(quantity_trajs['stress'])

        loss = gamma_stiffness * max_likelihood.mse_loss(stiffness_pred, target_dict['stiffness'])
...
def init_stiffness_and_stress_loss_function(energy_fn_template, box_tensor, kbT, N, target_dict):
    ...
    def loss_fn(quantity_trajs, weights):
        ...
        loss = gamma_stiffness * max_likelihood.mse_loss(stiffness_pred, target_dict['stiffness']) + \
               gamma_stress * max_likelihood.mse_loss(stress_tensor_mean, target_dict['stress'])
...
def init_stiffness_and_pressure_loss_function(energy_fn_template, box_tensor, kbT, N, target_dict):

    gamma_stiffness = 1.e-10
    # gamma_stress = 5.e-8
    # gamma_pressure_scalar = 5.e-7
    ...
    def loss_fn(quantity_trajs, weights):
        ...
        pressure_scalar_mean = reweighting_average(quantity_trajs['pressure_scalar'])
        # stress_tensor_mean = reweighting_average(quantity_trajs['stress'])

        loss = gamma_stiffness * max_likelihood.mse_loss(stiffness_pred, target_dict['stiffness']) +\
               gamma_pressure_scalar * max_likelihood.mse_loss(pressure_scalar_mean, target_dict['pressure_scalar'])

        predictions = {'pressure_scalar': pressure_scalar_mean, 'stiffness': stiffness_pred}
...
class PropagationBase(max_likelihood.MLETrainerTemplate):
    ...
    @opt_state.setter
    def opt_state(self, passed_state):
        self.state = self.state.replace(opt_state=passed_state)

    @property
    def all_optimizer(self):
        return self.optimizer

    @all_optimizer.setter
    def all_optimizer(self, passed_optimizer):
        self.optimizer = passed_optimizer
    ...
    def train(self, max_epochs, thresh=None, checkpoint_freq=None):
        if not isinstance(self.n_statepoints, onp.ndarray):
            assert self.n_statepoints > 0, ('Add at least 1 state point via '
                                            '"add_statepoint" to start training.')
        else:
            assert self.n_statepoints.shape[0] > 0, ('Statepoint array must have at least'
                                                     'one entry.')
