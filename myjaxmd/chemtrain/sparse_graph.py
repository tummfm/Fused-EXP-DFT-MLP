...
def cross_wrapper(r_ij_and_r_kj):
    """Takes in r_ij_and_r_kj which is a jnp array of the arrays
    r_ij and r_kj and computes the cross product. This is necessary
    for backprop."""
    return jnp.cross(r_ij_and_r_kj[0], r_ij_and_r_kj[1])


def dot_wrapper(r_ij_and_r_kj):
    """Takes in r_ij_and_r_kj which is a jnp array of the arrays
    r_ij and r_kj and computes the dot product. This is necessary
    for backprop."""
    return jnp.dot(r_ij_and_r_kj[0], r_ij_and_r_kj[1])
...
def angle(r_ij, r_kj):
    ...
    """
    # Mask collinear vectors
    mask_col = (jnp.sum(jnp.cross(r_ij, r_kj)) == 0)

    # Setting cross = 0. and dot = -1. yields theta = pi
    cross = jnp.linalg.norm(cross_wrapper(jnp.where(mask_col, jnp.array([jnp.array([0., 0.00000000001, 1.0]),
                                                                              jnp.array([0., 0., -1.])]),
                                                    jnp.array([r_ij, r_kj]))))

    dot = dot_wrapper(jnp.where(mask_col, jnp.array([jnp.array([0., 0., 1.0]),
                                                          jnp.array([0., 0., -1.])]), jnp.array([r_ij, r_kj])))
...
def sparse_graph_from_neighborlist(displacement_fn: Callable,
    ...
    print('WARNING: Stephan used default species zeros, changed this to ones to Ti pretrained trainer!')
    species = layers.canonicalize_species_ones(species, n_particles)
...
def sparse_graph_from_positions(box: jnp.ndarray,
                                   positions: jnp.ndarray,
                                   r_cutoff: jnp.array,
                                   species: jnp.array = None,
                                   max_edges: Optional[int] = None,
                                   max_triplets: Optional[int] = None,
                                   species_mask: jnp.array = None,
                                   precom_edge_mask: jnp.ndarray = None
                                   ) -> Tuple[SparseDirectionalGraph, bool]:
    """Constructs a sparse representation of graph edges and angles to save
    memory and computations over neighbor list.
    ...
    Args:
        displacement_fn: Jax_MD displacement function encoding box dimensions
        box: Box is passed instead of displacement_fn and displacement_fn is
             computed on the fly.
        positions: (N_particles, dim) array of particle positions
        # neighbor: Jax_MD neighbor list that is in sync with positions
        r_cutoff: Radial cutoff distance, below which 2 particles are considered
    ...
    Returns:
        ...
        SparseDirectionalGraph and whether max_edges or max_triplets overflowed.
    """
    # TODO (Ti-sebi) Remove neighbor list dependency and replace neighbor.idx
    # without using neighbor_fn
    #
    # # TODO might be worth updating this function to the new sparse-style
    # #  neighborlist in jax_md
    # assert neighbor.format.name == 'Dense', ('Currently only dense neighbor'
    ...
    # This neighbor list is the largest possible - TODO(Ti-Sebi) Improve to make it sparser?
    # Replace neighbor.idx with neighbor_idxs as neighbors are no more passed in this function
    n_particles = len(positions)
    true_len_molecule = jnp.count_nonzero(species)
    max_neighbors = n_particles - 1
    # TODO - Sebi: more efficient with ~(Stephan outer product)
    neighbor_idxs = jnp.array([onp.concatenate((onp.arange(0, i), onp.arange(i+1, n_particles)))
    ...
    # positions = scale_fn(positions)

    neighbor_displacement_fn = space.map_neighbor(displacement_fn)

    # compute pairwise distances
    # pos_neigh = positions[neighbor.idx]
    pos_neigh = positions[neighbor_idxs]
    pair_displacement = neighbor_displacement_fn(positions, pos_neigh)
    pair_distances = space.distance(pair_displacement)
    ...
    edge_idx_ji = jnp.where(pair_distances < r_cutoff, neighbor_idxs,
                            n_particles)

    # Mask all padded edges with passed precom_edge_mask
    edge_idx_ji = edge_idx_ji + precom_edge_mask
    edge_idx_ji = jnp.where(edge_idx_ji < true_len_molecule, edge_idx_ji, n_particles)

    # Original idea: Does not work to change dynamically
    # max_pos = len(positions)
    # non_zero = onp.count_nonzero(species)
    # edge_idx_ji = edge_idx_ji.at[:, non_zero-1:].set(max_pos)
    # edge_idx_ji = edge_idx_ji.at[non_zero:, :].set(max_pos)
...
def get_max_edges_max_triplets(r_cutoff, position_data, box):
    ...
    for (positions, cur_box) in zip(position_data, box):
        box_tensor = cur_box
        scale_fn = custom_space.fractional_coordinates_triclinic_box(cur_box)
        displacement_fn, _ = space.periodic_general(box_tensor)
...
def pad_positions(species, position_data):
    if not isinstance(position_data, list):
        n_snapshots = position_data.shape[0]
        position_data = [position_data[i] for i in range(n_snapshots)]
    else:
    ...
    all_pos = []
    for (cur_pos, cur_species) in zip(position_data, species):
        # box_tensor, scale_fn = custom_space.init_fractional_coordinates(cur_box)
        # displacement_fn, _ = space.periodic_general(box_tensor)
        # cur_pos = scale_fn(cur_pos)  # to fractional coordinates
...
def pad_forces_positions_species(species, position_data, forces_data, box):
    ...
    species_mask = []
    edge_mask = []
    # Convert positions to fractional, pad positions, pad forces, and species
    for (cur_pos, cur_box, cur_force, cur_species) in zip(position_data, box, forces_data, species):
        # box_tensor, scale_fn = custom_space.init_fractional_coordinates(cur_box)
        # displacement_fn, _ = space.periodic_general(box_tensor)
        # cur_pos = scale_fn(cur_pos)  # to fractional coordinates
        pad_len = max_pos - len(cur_pos)
        ...
        # species mask
        cur_species = onp.concatenate((cur_species, onp.zeros(pad_len, dtype=int)))
        species_mask.append(cur_species)

        # edge_mask
        temp_edge_mask = onp.zeros((max_pos, max_pos-1))
        non_zero = onp.count_nonzero(cur_species)
        num_zeros = max_pos - non_zero
        temp_edge_mask[:, non_zero-1:] = max_pos
        temp_edge_mask[non_zero:, :] = max_pos
        edge_mask.append(temp_edge_mask)


    # Turn list into np array(are padded, thus each list entry has same size)
    all_pos = jnp.array(all_pos)
    all_forces = jnp.array(all_forces)
    species_mask = jnp.array(species_mask, dtype=int)
    force_mask = all_forces.astype(dtype=bool).astype(dtype=int)
    edge_mask = jnp.array(edge_mask, dtype=int)

    return all_pos, all_forces, species_mask, force_mask, edge_mask
